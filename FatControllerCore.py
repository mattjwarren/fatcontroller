...
class FatControllerCore:
    '''Core business logic without GUI dependencies - testable in headless mode'''
    
    def __init__(self, init_gui=True):
        """Initialize FatController core.
        
        Args:
            init_gui: If True, initialize full GUI. If False, only init business logic (for testing)
        """
        # Conditional GUI initialization
        if init_gui:
            # Import here to avoid issues when GUI not needed
            import ttkbootstrap as ttk
            # We can't call super().__init__() since we don't inherit from ttk.Window
            # The actual GUI window will be created by FatController subclass
            pass
        
        # Set GUI flag
        self._init_gui = init_gui
        ...
        if init_gui:
            ...
            self.OutBook = MagicMock()
            self.ShellEntry = MagicMock()
            self.ShellEntry.get = MagicMock(return_value="")
            self.FirstPageTextCtrl = MagicMock()
            self.ShellTextCtrl = MagicMock()
            # Mock RHS panel widgets
            self.ObjectTypeVar = MagicMock()
            self.ObjectListbox = MagicMock()
            self.ObjectListbox.curselection = MagicMock(return_value=tuple())
            self.ObjectListbox.get = MagicMock(return_value="")
            self.ObjectListbox.delete = MagicMock()
            self.ObjectListbox.insert = MagicMock()
            self.ConfigSelectNotebook = MagicMock()
    ...
    # Testable Business Logic Methods (no GUI dependencies)
    
    def refresh_object_data(self, object_type, entity_type_filter=None):
        """
        Get list of object names for display (testable, no GUI).
        
        Args:
            object_type: Type of objects ('Entities', 'Daemons', 'Scripts', 'Aliases', 'Substitutions')
            entity_type_filter: Optional filter for entity type (e.g., 'TSM', 'SSH')
        ...
        try:
            if object_type == 'Entities':
                ...
                if entity_type_filter:
                    items = [name for name, obj in all_entities.items() 
                            if obj.getentitytype() == entity_type_filter]
                else:
                    items = list(all_entities.keys())
                    
            elif object_type == 'Daemons':
                items = list(self.DaemonManager.getDaemons().keys())
            elif object_type == 'Scripts':
                items = list(self.scripts.keys())
            elif object_type == 'Aliases':
                items = list(self.aliases.keys())
            elif object_type == 'Substitutions':
    ...
    def get_filtered_entities(self, entity_type_filter=None):
        """Get dictionary of entities, optionally filtered by type (testable, no GUI)."""
        all_entities = self.EntityManager.getentitylist()
        if entity_type_filter:
            return {name: obj for name, obj in all_entities.items() 
                   if obj.getentitytype() == entity_type_filter}
        return all_entities
    
    def remove_object_logic(self, object_type, object_name):
        """
        Handle object deletion logic (testable, no GUI).
        ...
        try:
            if object_type == 'Entities':
                self.EntityManager.delete(object_name)
            elif object_type == 'Daemons':
                self.DaemonManager.deleteDaemon(object_name)
            elif object_type == 'Scripts':
                if object_name in self.scripts:
                    del self.scripts[object_name]
            elif object_type == 'Aliases':
                if object_name in self.aliases:
                    del self.aliases[object_name]
            elif object_type == 'Substitutions':
                if object_name in self.substitutions:
                    del self.substitutions[object_name]
            return True
        except Exception as e:
    ...
    def refresh_object_list(self):
        """Update GUI listbox with current objects (calls testable core method)."""
        self.ObjectListbox.delete(0, tk.END)
        obj_type = self.ObjectTypeVar.get()
        
        # Get entity type filter if applicable
        entity_type_filter = None
        if obj_type == 'Entities' and hasattr(self, 'EntityTypeVar'):
            entity_type_filter = self.EntityTypeVar.get()
        
        # Get data from testable core method
        items = self.refresh_object_data(obj_type, entity_type_filter)
    ...
    def remove_selected_objects(self):
        """Remove selected objects from listbox (calls testable core method)."""
        cur_selection = self.ObjectListbox.curselection()
        if not cur_selection:
            return
        ...
        # Use testable core method for deletion
        for name in names:
            self.remove_object_logic(obj_type, name)
