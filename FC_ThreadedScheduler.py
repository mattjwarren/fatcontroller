import  FC_ScheduledTaskHandler
###########
...
class ThreadedScheduler(Thread):
    ...
    def addTimedAction(self, time, task, name):
        ...
        if not handle:
            handle =FC_ScheduledTaskHandler.ScheduledTaskHandler(self, time, 0, task, name)
        else:
    ...
    def addActionOnDemand(self, task, name):
        ...
        handle = self.unregisterTask(name)
        if not handle:
            handle =FC_ScheduledTaskHandler.ScheduledTaskHandler(self,time.time(), 0, task, name)
        else:
            handle.reset(time(), 0, task, 1)
    ...
    def addPeriodicAction(self, start, period, task, name):
        ...
        handle = self.unregisterTask(name)
        if not handle:
            handle =FC_ScheduledTaskHandler.ScheduledTaskHandler(self, start, period, task, name)
        else:
